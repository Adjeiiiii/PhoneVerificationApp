...
public interface SurveyLinkPoolRepository extends JpaRepository<SurveyLinkPool, UUID> {
    ...
    interface ClaimResult {
        ...
        String getLinkPublicUid();
    }

    @Query(value = "SELECT link_id AS linkId, link_url AS linkUrl, short_link_url AS shortLinkUrl, link_public_uid AS linkPublicUid FROM claim_available_link(:batchLabel)", nativeQuery = true)
    Optional<ClaimResult> claimAvailable(@Param("batchLabel") String batchLabel);

    @Query(value = "SELECT link_id AS linkId, link_url AS linkUrl, short_link_url AS shortLinkUrl, link_public_uid AS linkPublicUid FROM claim_available_link(NULL)", nativeQuery = true)
    ...
    boolean existsByLinkPublicUid(String linkPublicUid);
}
